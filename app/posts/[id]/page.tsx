...
const posts = [
  {
    ...
    readTime: "5 min read",
    image:
      "https://images.unsplash.com/photo-1508672019048-805c876b67e2?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2340&q=80",
    category: "Lifestyle",
  },
...
export default function BlogPost({
  ...
  useEffect(() => {
    ...
    const foundPost = allPosts.find(p => p.id === parseInt(params.id))
    setPost(foundPost)
  }, [params.id])
        ...
        <div className="max-w-3xl mx-auto">
          ...
          >
            <Image
              src={
                post?.image ||
                "https://images.unsplash.com/photo-1501504905252-473c47e087f8?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2340&q=80"
              }
              alt={post?.title || ""}
              width={800}
              height={400}
                  ...
                  .map((paragraph: string, index: number) => {
                    if (paragraph.startsWith("## ")) {
                      return (
                        <h2
                          key={index}
                          className="text-3xl font-semibold mb-6 font-serif"
                        >
                          {paragraph.replace("## ", "")}
                        </h2>
                      );
